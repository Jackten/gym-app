-- Pelayo Wellness launch hardening
-- Reasserts stricter booking guards after migration-order drift and blocks
-- past/cross-midnight create or reschedule attempts.
...
begin
  ...
  if new.slot_date < current_date
    or (new.slot_date = current_date and new.start_time <= localtime) then
  ...
  select
    count(*) filter (
      where slot_date < current_date
        or (slot_date = current_date and new_start_time <= localtime)
    ),
